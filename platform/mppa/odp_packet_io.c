...
{
	return &pktio_tbl.entries[index];
}
	...
	for (i = 0; i < ODP_CONFIG_PKTIO_ENTRIES; ++i) {
		...
		if (is_free(entry)) {
			...
			if (is_free(entry)) {
				...
				return (odp_pktio_t)entry; /* return with entry locked! */
			}
		}
	}
				     ...
				     const odp_pktio_param_t *param)
{
	...
	for (pktio_if = 0; pktio_if_ops[pktio_if]; ++pktio_if) {
		ret = pktio_if_ops[pktio_if]->open(hdl, pktio_entry, name, pool);
...
static void destroy_in_queues(pktio_entry_t *entry, int num)
{
	...
	for (i = 0; i < num; i++) {
		if (entry->s.in_queue[i].queue != ODP_QUEUE_INVALID) {
			rc = odp_queue_destroy(entry->s.in_queue[i].queue);
			ODP_ASSERT(rc == 0);
			entry->s.in_queue[i].queue = ODP_QUEUE_INVALID;
		}
	}
...
{
	odp_packet_t pkt = _odp_packet_from_buffer_hdr(buf_hdr);
	int len = 1;
	int nbr;
		     ...
		     int num)
{
	...
	for (i = 0; i < num; ++i)
		pkt_tbl[i] = _odp_packet_from_buffer_hdr(buf_hdr[i]);
		    ...
		    odp_pktio_stats_t *stats)
{
	...
	if (!entry->s.ops->stats_reset) {
		/* No reset, we need to manually remove the last
		   reset stats from the counters */
		stats->in_octets -= entry->s.stats.in_octets;
		stats->in_ucast_pkts -= entry->s.stats.in_ucast_pkts;
		stats->in_discards -= entry->s.stats.in_discards;
		stats->in_errors -= entry->s.stats.in_errors;
		stats->in_unknown_protos -= entry->s.stats.in_unknown_protos;
		stats->out_octets -= entry->s.stats.out_octets;
		stats->out_ucast_pkts -= entry->s.stats.out_ucast_pkts;
		stats->out_discards -= entry->s.stats.out_discards;
		stats->out_errors -= entry->s.stats.out_errors;
	}
	unlock_entry(entry);
...
int odp_pktio_stats_reset(odp_pktio_t pktio)
{
	...
	} else if (entry->s.ops->stats) {
		ret = entry->s.ops->stats(entry, &entry->s.stats);
	}
	unlock_entry(entry);
			...
			odp_packet_t pkt_table[], unsigned len)
{
	...
	for (i = 0; i < len; i++) {
		...
		const uint32_t pkt_len = packet_len(pkt_hdr);
		uint32_t seglen;
		void *segptr;
		odp_pool_t pool;

		segptr = packet_map(pkt_hdr, 0, &seglen);
		if (cls_classify_packet(pktio_entry, segptr, pkt_len,
					seglen, &pool, pkt_hdr, 0)) {
			/* This packet is not to be classified */
			continue;
		}

		if (pool != pktio_entry->s.pool) {
			odp_packet_t newpkt =
				odp_packet_copy(pkt_table[i], pool);
