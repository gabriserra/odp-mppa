 ...
 *  @{
 */
#define _ODP_BASENAME(filepath) ({ const char *___file = filepath;  \
			const char *___ptr = strrchr(___file, '/'); \
			___ptr ? (___ptr + 1) : ___file;})
...
#define ODP_UNIMPLEMENTED()								\
	odp_global_data.log_fn(ODP_LOG_UNIMPLEMENTED,					\
			       ...
			       _ODP_BASENAME(__FILE__), __LINE__, __func__)
/**
...
#define ODP_LOG(level, fmt, ...) \
	odp_global_data.log_fn(level, "%s:%d:%s():" fmt,		\
			       _ODP_BASENAME(__FILE__),			\
