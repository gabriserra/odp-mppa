...
/** Mark ETH Rpc has available */
#define HAS_ODP_RPC_ETH

/** Version of the ETH CoS */
#define MPPA_RPC_ODP_ETH_VERSION 0x7
...
typedef union {
	struct {
		uint8_t ifId : 3; /* 0-3, 4 for 40G */
		uint8_t enabled : 1;
		uint8_t no_wait_link : 1;/**< If set, ETH_STATE(1) does not wait for the link to be up */
		uint8_t verbose : 1;
	};
	mppa_rpc_odp_inl_data_t inl_data;
