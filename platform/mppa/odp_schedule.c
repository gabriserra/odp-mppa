...
ODP_STATIC_ASSERT(ODP_SCHED_PRIO_LOWEST == (SCHEDULE_NUM_PRIO - 1),
		  "lowest_prio_does_not_match_with_num_prios");

ODP_STATIC_ASSERT((ODP_SCHED_PRIO_NORMAL > 0) &&
		  (ODP_SCHED_PRIO_NORMAL < (SCHEDULE_NUM_PRIO - 1)),
...
typedef uint32_t pri_mask_t;

ODP_STATIC_ASSERT((8 * sizeof(pri_mask_t)) >= SCHEDULE_QUEUES_PER_PRIO,
...
typedef struct {
	odp_queue_t    pri_queue[SCHEDULE_NUM_PRIO][SCHEDULE_QUEUES_PER_PRIO];
	pri_mask_t     pri_mask[SCHEDULE_NUM_PRIO];
	...
	odp_pool_t     pool;
	odp_shm_t      shm;
	uint32_t       pri_count[SCHEDULE_NUM_PRIO][SCHEDULE_QUEUES_PER_PRIO];
...
static int schedule_init_global(void)
{
	...
	for (i = 0; i < SCHEDULE_NUM_PRIO; i++) {
		...
		for (j = 0; j < SCHEDULE_QUEUES_PER_PRIO; j++) {
			name[10] = '0' + j / 10;
			name[11] = '0' + j - 10*(j / 10);
...
{
	return ((SCHEDULE_QUEUES_PER_PRIO - 1) & queue_index);
}

static odp_queue_t pri_set(int id, int prio)
{
	odp_spinlock_lock(&sched->mask_lock);
	UNCACHED_OP32(sched->pri_mask[prio], mask, mask | (1 << id));
	UNCACHED_OP32(sched->pri_count[prio][id], count, count + 1);
...
static void pri_clr(int id, int prio)
{
	...
	count = UNCACHED_OP32(sched->pri_count[prio][id], _count, _count - 1);
		       ...
		       unsigned int max_num, unsigned int max_deq)
{
	...
	for (i = 0; i < SCHEDULE_NUM_PRIO; i++) {
		...
		id = (sched_local.thr + offset) & (SCHEDULE_QUEUES_PER_PRIO - 1);

		for (j = 0; j < SCHEDULE_QUEUES_PER_PRIO; j++, id++) {
			odp_queue_t  pri_q;
			int num;
			int grp;
			int ordered;
			...
			if (id >= SCHEDULE_QUEUES_PER_PRIO)
				id = 0;
