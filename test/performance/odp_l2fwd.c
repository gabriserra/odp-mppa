 ...
 * @retval -1 on failure
 */
static odp_pktio_t create_pktio(const char *dev, int idx, int num_rx, int num_tx,
			odp_pool_t pool)
{
	odp_pktio_t pktio;
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < if_count; ++i) {
		...
		if (odp_pktio_mac_addr(gbl_args->pktios[n_pktios].pktio,
				       gbl_args->port_eth_addr[n_pktios].addr,
		...
		if (gbl_args->appl.dst_change) {
			...
			if (gbl_args->appl.addr_count) {
				memcpy(&new_addr, &gbl_args->appl.addrs[n_pktios],
	...
	gbl_args->pktios[n_pktios].pktio = ODP_PKTIO_INVALID;
